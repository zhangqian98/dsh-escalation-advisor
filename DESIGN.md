...
Case packets use explicit section priorities, bounded UTF-8 JSON, credential redaction, runtime outcome classifications, and call/result IDs. They retain capabilities independently from truncatable evidence. Continuous packets track a reviewed sequence and omit old activity. A user's root or requester inbox insertion changes the revision before the new message is admitted, closing the gap where an old background review finishes while new intent is queued.
