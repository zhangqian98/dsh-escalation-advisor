...
Case packets still select relevant task evidence, summarize individual records, and redact secrets. They are no longer pruned to an independent total byte budget, and generated plain-text output is no longer cut to an Advisor-specific size. Individual evidence sections are bounded by entry count instead: validation keeps its trigger-related and failing checks plus a short recent tail of successes, recent activity keeps the newest calls, and failures keep the newest ones — pinning a failure is possible only when the tracker assigned its command a validation key that a trigger signal also carries, so an ordinary non-zero exit that helped cause the trigger can still fall outside the cap. Successful `run_code` wrappers are omitted when their concrete PTC sub-dispatches are present; failed or unfinished wrappers remain. Tool results have one canonical copy in `tool_activity`, while `recent_tail` carries only conversational framing. `failures` and `validation` reference that canonical activity by `call_id` instead of repeating commands and output. Truncation would hide how much evidence a task actually produced, so the packet also carries a `validation_summary` aggregate (total, retained, omitted, succeeded, failed, other, relevant) describing the pre-cap history. Escalation has no review cursor, so without those caps a long task would resend its whole transcript on every escalation.
