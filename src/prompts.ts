...
export function toolGuidance(mode: AdvisorMode): string {
  if (mode === 'continuous') return `## Strong advisor\nA stronger shadow reviewer checks natural turn boundaries. You may also call \`ask_advisor\` when you need an immediate second opinion before the turn ends. Do not call it routinely or duplicate a question the shadow reviewer has already answered.`
  if (mode === 'escalate') return `## Strong advisor\nYou have \`ask_advisor\` for a stronger second opinion. Use it proactively when evidence conflicts, a high-impact design decision is uncertain, or you recognize that your current hypothesis may be wrong. An automatic escalation gate also watches repeated failures and may inject advisor guidance when you do not notice that you are stuck. Do not call the advisor for routine implementation work.`
  return `## Strong advisor\nYou have \`ask_advisor\` for an on-demand stronger second opinion. Use it for genuine uncertainty, conflicting evidence, repeated failed attempts, or high-impact decisions. Do not call it for routine implementation work.`
}
